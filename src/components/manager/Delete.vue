  ...
  <div class="bg-white dark:bg-dark-grey rounded-lg">
    <div class="p-6 flex flex-col gap-6">
      <div class="flex justify-between items-center">
        <h4 class="text-red font-bold text-lg">
          Delete this {{ managerStore.delete.board ? 'board' : 'task' }}
        </h4>
      </div>
      <p class="text-medium-grey text-sm">{{ warning }}</p>
      <div class="flex flex-col gap-4">
        <ButtonDelete @click.stop="onDelete">
          Delete
        </ButtonDelete>
        <ButtonSecondaryLarge @click.stop="onCancel">
          Cancel
